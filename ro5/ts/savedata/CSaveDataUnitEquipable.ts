...
export class CSaveDataUnitEquipable extends CSaveDataUnitBase {
    ...
    // オーバーライドされた parse 関数
    parse(dataText: any, bitOffset: any) {
        ...
        if (this.getProp("version") == 1n) {	// version 1 のセーブデータを version 2 以上のプロパティ定義で読み込むとデータが壊れるので差分修正する
            ...
            if (CSaveDataUnitEquipable.deprecatedItemIdChangeArray == null) {
                ...
                // @ts-expect-error TS(2322): Type 'any[]' is not assignable to type 'null'.
     ...
     */
    convertFromOldFormat(dataTextWork: any, bitOffset: any, ...args: any[]) {
     ...
     * このメソッドはセーブ時のみ呼び出される
     */
    encodeToURL(dataTextWork: any, bitOffset: any) {
        // バージョンを修正
