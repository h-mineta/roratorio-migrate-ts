import { CMultiValueMapper } from "./CMultiValueMapper";
import { CSaveDataConst } from "./CSaveDataConst";
import { CSaveDataPropInfo } from "./CSaveDataPropInfo";
...
export class CSaveDataUnitBase {
     ...
     */
    // @ts-expect-error TS(7008): Member 'parsedMap' implicitly has an 'any' type.
    parsedMap;
    ...
    parse(dataText, bitOffset) {
        // すべてのプロパティを処理する
        // @ts-expect-error TS(2339): Property 'propNames' does not exist on type 'Funct... Remove this comment to see the full error message
    ...
    parseProp(propName, dataText, bitOffset) {
        ...
        if (extraBits != 0) {
            ...
            // @ts-expect-error TS(2304): Cannot find name 'CSaveDataConverter'.
            const extraNum: any = CSaveDataConverter.ConvertStoNMIG(extraChar);
            // はみ出し部分のみ取得
            // @ts-expect-error TS(2737): BigInt literals are not available when targeting l... Remove this comment to see the full error message
            extraValue = extraNum & ((1n << toSafeBigInt(extraBits)) - 1n);
        }
        // 最終的な値を合成
        // @ts-expect-error TS(2304): Cannot find name 'toSafeBigInt'.
     ...
     */
    // @ts-expect-error TS(7006): Parameter 'dataTextWork' implicitly has an 'any' t... Remove this comment to see the full error message
    ...
    convertFromOldFormat(dataTextWork, bitOffset, ...args) {
        ...
        // @ts-expect-error TS(2339): Property 'type' does not exist on type 'Function'.
    ...
    SetUpAsDefault() {
        ...
        // @ts-expect-error TS(2339): Property 'version' does not exist on type 'Functio... Remove this comment to see the full error message
