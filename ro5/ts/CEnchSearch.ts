class enchSearch {
    ...
    constructor() {
        ...
        if (g_constDataManager.enchListDataManager) {
            this.itemIds = g_constDataManager.enchListDataManager.reverseResolveArrayItemId; //ratorioから取得
        } else {
            this.itemIds = []; // fallback to empty array or handle as needed
            console.warn('enchListDataManager is null');
        }
        this.CardObjNew = CardObjNew;//ratorioから取得

        //エンチャントとアイテムidを紐づけした配列を生成(ratorioの処理を利用)
        this.itemIds.forEach((enchSetId: any, itemId: any) => {
            if (!enchSetId) return;
            var enchListId = 0;
            // @ts-expect-error TS(2304): Cannot find name 'CONST_DATA_KIND_ENCHANT_LIST'.
            ...
            for (let idxEnchList = 0; idxEnchList < enchListIdArray.length; idxEnchList++) {
                ...
                // サブ関数をコールしてデータ配列を収集
                // @ts-expect-error TS(2304): Cannot find name 'RebuildCardSelectSubCollectEnchL... Remove this comment to see the full error message
    ...
    resetEnchSearch(mode: any) {
        ...
        for (let d of Array.from(dataInsert)) d.remove();
