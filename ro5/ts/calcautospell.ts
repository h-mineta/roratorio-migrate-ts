...
// オートスペル設定　最大数
export const AUTO_SPELL_SETTING_COUNT: number = 20;


// オートスペル系スキル　最大数
export const AUTO_SPELL_SKILL_COUNT_MAX: number = 40;


// オートスペル発動率（千分率）
export const AUTO_SPELL_PROB_ARRAY: any[] = [
];

for (let idx: number = 0, inc: number = 1; idx <= 1000; idx += inc) {
...
for (let idx: number = 0; idx < AUTO_SPELL_SKILL_COUNT_MAX; idx++) {
 ...
 * オートスペルの計算.
 */
export function AS_Calc(charaData: any, specData: any, mobData: any, attackMethodConfArray: any[], battleCalcInfo: any) {
    ...
    let skillLvOld = 0;
    let skillKind = 0;
    let attackMethodConfArrayAS = null;
    let clonedCalcInfo = null;
    let battleCalcResultAll = null;

    let funcAddAS = function () {
        n_AS_SKILL.push([]);
    ...
    for (idx = 0; idx < AUTO_SPELL_SKILL_COUNT_MAX; idx++) {
        n_AS_DMG[idx][0] = 0;
        n_AS_DMG[idx][1] = 0;
        n_AS_DMG[idx][2] = 0;
    ...
    let itemCount = 0;
    let skillLvEternalChain = 0;
    let sereKind;
    let sereMode;
    let skillLvDoubleCasting;
    ...
    // 自動鷹の場合
    else if ((n_A_ActiveSkillIdNum == 0) && (n_A_WeaponType == ITEM_KIND_BOW) && (skillLvBlitz > 0)) {
    ...
    switch (n_A_ActiveSkillIdNum) {
        ...
        case SKILL.ID_LIGHTNING_BOLT:
            ...
            if (skillLvDoubleCasting > 0) {
                funcAddAS();
                n_AS_SKILL[idx][0] = n_A_ActiveSkillIdNum;
