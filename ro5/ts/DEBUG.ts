 ...
 */
function WriteConsoleLog(message: string): void {
    console.log(message);
}
...
// データ作成フラグ
let _DATA_CREATE_MODE = (_DEBUG && true);

// デバッグ用ファイル読み込みチェックフラグ
let g_bDebugFileIncluded = false;
...
// 魔法ダメージ計算順序検証フラグ
let _MAGIC_CALC_INSPECTION = (_DEBUG && false);

// エンチャントリストデータ形式移行フラグ
let _ENCH_LIST_MIG = true;

// Lv200解放アップデート適用フラグ
let _APPLY_UPDATE_LV200 = true;

// Lv200解放アップデート　クリティカル計算方式変更　適用フラグ
let _APPLY_UPDATE_LV200_CRITICAL = true;
...
// パース利用フラグ
let _DATA_PARSE_MODE = (_DEBUG && true);

// パーサ用ファイル読み込みチェックフラグ
let g_bParserFileIncluded = false;
...
// データ移行フラグ
let _DATA_MIGRATION_MODE = (_DEBUG && false);

// データ移行フラグ（新プロセス）
// ・移行処理のために新設された処理
// ・既存処理には影響を与えない
// ・移行完了後も残る処理
let _ENABLE_MIGRATION_BLOCK_NEW_PROCESS = (_DATA_MIGRATION_MODE && true);

// データ移行フラグ（過渡期の補正）
// ・移行処理のために新設された処理
// ・既存処理には影響を与えない
// ・移行完了後は残らない処理
let _ENABLE_MIGRATION_BLOCK_TRANSIT = (_DATA_MIGRATION_MODE && true);

// データ移行テスト用カバレッジ収集フラグ
let _ENABLE_MIGRATION_COVERAGE = (_DATA_MIGRATION_MODE && true);

// テスト設定変更中フラグ
let _TEST_SETTINGS_APPLYING = false;
...
let g_bMigrationFileIncluded = false;
 ...
 */
function IsMigrationedItemId(itemId: string) {
