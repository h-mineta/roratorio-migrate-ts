...
/** 一次職支援　設定値配列 */
let g_confDataIchizi: any = null;

/** 二次職支援　設定値配列 */
let g_confDataNizi: { [x: string]: number; } | null = null;

/** 三次職支援　設定値配列 */
let g_confDataSanzi: any = null;

/** 四次職支援　設定値配列 */
let g_confDataYozi: any = null;

/** 一次職支援　設定欄オブジェクト */
let g_objCharaConfIchizi: any = null;

/** 二次職支援　設定欄オブジェクト */
let g_objCharaConfNizi: any = null;

/** 三次職支援　設定欄オブジェクト */
let g_objCharaConfSanzi: any = null;

/** 四次職支援　設定欄オブジェクト */
let g_objCharaConfYozi: any = null;

/** 時限効果設定　設定数 */
const TIME_ITEM_CONF_COUNT: number = 20;
...
function ResetConfDataAllMIG(bAsOnLoad) {
    ...
    if (!bAsOnLoad) {
        ...
        // データ管理インスタンス生成
        // @ts-expect-error TS(2304): Cannot find name 'g_dataManagerMobConfInput'.
...
function __DIG3(value: number): string {
    ...
    if (HtmlGetObjectCheckedById("OBJID_CHECK_DIGIT3", false)) {
        return DivideDigits3(value).toString();
    }
